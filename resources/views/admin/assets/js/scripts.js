$(function() {
    ...
    $(".nav_tabs_item_link").click(function(event) {
        ...
        $(componentNav)
            .find(".nav_tabs_item .active")
            .removeClass("active");

        $(this).addClass("active");

        $(componentNav)
            .find(".nav_tabs_content")
            .children()
            .hide();
...
//====================================================================
//SCRIPT PARA APRESENTAR INPUTS CASO SEJA SELECIONADO ALGUM SELECT
//SCRIPT PARA CASADO SOLTERIO SEPARADO VIUVO || CONJUGE
//PARA QUE O SCRIPT NAO SEJA EXECUTADO EM TODAS AS PAGINAS. Usamos a condição com typeof, se for diferente de indefinido é para ser executado
//====================================================================
function normalizeSpouze() {
    //se ele for diferenete de indefinido, executamos o script
    if (typeof 'select[name="civil_status"]' !== "undefined") {
        if (
            $('select[name="civil_status"]').val() === "married" ||
            $('select[name="civil_status"]').val() === "separated"
        ) {
            //caso essas duas opçoes estejam seecionadas
            //colocaremos a propriedade disable como false para liberar os campos.
            //pegamos a classe principal do conjuge, content_spouse e dentro dele pegamos todos inputs, tmb os selects
            $(".content_spouse input, .content_spouse select").prop(
                "disabled",
                false
            );
        } else {
            //colocaremos a propriedade disable como true para bloquear os campos, caso seja, selecionado solterio/divorciado/viuvo.
            $(".content_spouse input, .content_spouse select").prop(
                "disabled",
                true
            );
...
//para que a funcao seja executado ao carregar a pagina:
normalizeSpouze();

//para que o gatilho dispare no momento que o select for alterado:
